...
from src.enhanced_reranker import get_business_reranker
...
class QuestionsProcessor:
    def __init__(
        ...
        if self.use_enhanced_reranker:
            self.enhanced_reranker = get_business_reranker()
        else:
    ...
    def get_answer_for_company(self, company_name: str, question: str, schema: str) -> dict:
        ...
        if self.query_rewriter:
            try:
                tqw = time.time()
                rewrite_result = self.query_rewriter.auto_rewrite(question)
                rewritten_question = rewrite_result.get('rewritten_query', question)
                query_rewrite_info = {
                ...
                tqw2 = time.time()
                print(f"[计时] [get_answer_for_company] 查询改写耗时: {tqw2-tqw:.2f} 秒")
                print(f"[查询改写] 原始: {question}")
                print(f"[查询改写] 改写后: {rewritten_question}")
                if rewrite_result.get('query_type'):
                    print(f"[查询改写] 类型: {rewrite_result.get('query_type')}, 置信度: {rewrite_result.get('confidence')}")
            except Exception as e:
                print(f"[查询改写] 改写失败，使用原始查询: {e}")
                rewritten_question = question
        ...
        if self.full_context:
            ...
            retrieval_results = retriever.retrieve_by_company_name(
                ...
                query=rewritten_question,  # 使用改写后的查询
        ...
        # 应用增强重排序
        if self.enhanced_reranker and retrieval_results:
            try:
                tr = time.time()
                retrieval_results = self.enhanced_reranker.rerank(
                    rewritten_question, 
                    retrieval_results, 
                    top_k=self.top_n_retrieval
                )
                tr2 = time.time()
                print(f"[计时] [get_answer_for_company] 增强重排序耗时: {tr2-tr:.2f} 秒")
            except Exception as e:
                print(f"[增强重排序] 重排序失败，使用原始结果: {e}")
        ...
        if query_rewrite_info:
            answer_dict["query_rewrite_info"] = query_rewrite_info
        
        # 将检索结果作为引用来源返回给调用方
        retrieval_refs = []
        
        # 先收集所有原始distance值，用于归一化
        raw_distances = []
        for r in retrieval_results[:5]:
            if isinstance(r, dict):
                ...
                if rerank_score is not None:
                    raw_distances.append(0.0)  # 有rerank_score就不需要distance了
                elif similarity is not None:
                    raw_distances.append(0.0)
                else:
                    raw_distances.append(float(r.get('distance', 0)))
        
        # 归一化distance为相似度 (没有rerank_score时用)
        min_dist = min(raw_distances) if raw_distances else 0
        max_dist = max(raw_distances) if raw_distances else 1
        
        for i, r in enumerate(retrieval_results[:5], 1):
            text = r.get('text', r.get('content', ''))
            page = r.get('page', r.get('metadata', {}).get('page', 0)) if isinstance(r, dict) else 0
            ...
            if isinstance(r, dict):
                ...
                if rerank_score is not None:
                    ...
                    if isinstance(similarity, str):
                        score = float(similarity.replace('%', '')) / 100.0
                    else:
                        score = float(similarity)
                else:
                    # distance 越小越相似，归一化到 0-1 范围
                    dist = float(distance)
                    if max_dist > min_dist:
                        score = max(0.0, min(1.0, 1.0 - (dist - min_dist) / (max_dist - min_dist)))
                    else:
                        score = 1.0 if dist == 0 else max(0.0, 1.0 - dist)
            else:
                score = 0
        ...
        answer_dict["retrieval_results"] = retrieval_refs
    ...
    def _create_answer_detail_ref(self, answer_dict: dict, question_index: int) -> str:
        ...
        with self._lock:
            self.answer_details[question_index] = {
                ...
                "query_rewrite_info": answer_dict.get('query_rewrite_info'),
    ...
    def process_single_question(self, question: str, kind: str = "string", company_name: Optional[str] = None):
        """
        单条问题推理，返回结构化答案。
        kind: 支持 'string'、'number'、'boolean'、'names' 等
        company_name: 可选，直接指定公司名，绕过自动识别
        """
        t0 = time.time()
        print("[计时] [单问] 开始公司名抽取 ...")
        # 公司名抽取

...
class Pipeline:
    ...
    def export_reports_to_markdown(self, file_name, local_file_path=None):
        """
        ...
        :param local_file_path: 本地PDF文件的完整路径，可选。如果提供，会先上传到Mineru服务器
        """
        # 调用 pdf_mineru 获取 task_id 并下载、解压
        print(f"开始处理: {file_name}")
        task_id = pdf_mineru.get_task_id(file_name, local_file_path)
    ...
    def process_questions(self):
        ...
        processor = QuestionsProcessor(
            ...
            use_enhanced_reranker=self.run_config.use_enhanced_reranker
        )
    ...
    def answer_single_question(self, question: str, kind: str = "string", company_name: Optional[str] = None):
        """
        ...
        # 从 processor.answer_details 或 answer 中提取检索结果
        retrieval_results = answer.get("retrieval_results", [])
        if not retrieval_results and processor.answer_details:
            detail = processor.answer_details[0] if processor.answer_details else {}
            retrieval_results = detail.get("retrieval_results", [])
        answer["retrieval_results"] = retrieval_results
        
        # 提取 query_rewrite_info
        qr_info = answer.get("query_rewrite_info")
        if not qr_info and processor.answer_details:
            detail = processor.answer_details[0] if processor.answer_details else {}
            qr_info = detail.get("query_rewrite_info")
        if qr_info:
            answer["query_rewrite_info"] = qr_info
...
max_config = RunConfig(
    ...
    parallel_requests=4,
    submission_file=True,
    pipeline_details="Custom pdf parsing + vDB + Router + Parent Document Retrieval + Query Rewrite + Enhanced Rerank + SO CoT; llm = qwen-turbo",
...
# 新配置：启用查询改写和增强重排序
enhanced_config = RunConfig(
    use_serialized_tables=False,
    parent_document_retrieval=True,
    llm_reranking=False,
    parallel_requests=4,
    submission_file=True,
    pipeline_details="Enhanced RAG with Query Rewrite + Business Reranker; llm = qwen-turbo",
    answering_model="qwen-turbo-latest",
    config_suffix="_enhanced",
    use_query_rewrite=True,
    use_enhanced_reranker=True
)
...
if __name__ == "__main__":
    ...
    # 4. 将PDF转化为纯markdown文本（使用本地文件上传）
    print('4. 将PDF转化为纯markdown文本')
    pdf_file_name = '【财报】中芯国际：中芯国际2024年年度报告.pdf'
    local_pdf_path = root_path / "pdf_reports" / pdf_file_name
    print(f"本地PDF路径: {local_pdf_path}")
    # 取消注释以下行以启用PDF转Markdown（需要Mineru API）
    pipeline.export_reports_to_markdown(pdf_file_name, local_file_path=str(local_pdf_path)) 

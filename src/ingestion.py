...
class VectorDBIngestor:
    ...
    def _get_embeddings(self, text: Union[str, List[str]], model: str = "text-embedding-v1") -> List[float]:
        ...
        total_batches = (len(text_chunks) + MAX_BATCH_SIZE - 1) // MAX_BATCH_SIZE
        
        from tqdm import tqdm
        import time
        
        for batch_idx in tqdm(range(total_batches), desc="向量化进度", unit="批次"):
            i = batch_idx * MAX_BATCH_SIZE
        ...
        print(f"向量化完成！共处理 {len(embeddings)} 个文本块")
        return embeddings
